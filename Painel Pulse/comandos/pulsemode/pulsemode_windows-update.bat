...
echo Set-ItemProperty -Path $doPath -Name 'DODownloadMode' -Value 0 -Type DWord -Force
...
echo Remove-ItemProperty -Path $wuPath -Name 'DisableWindowsUpdateAccess' -Force -ErrorAction SilentlyContinue
echo Remove-ItemProperty -Path $wuPath -Name 'SetDisableUXWUAccess' -Force -ErrorAction SilentlyContinue
echo Set-ItemProperty -Path $wuPath -Name 'ExcludeWUDriversInQualityUpdate' -Value 1 -Type DWord -Force
echo Set-ItemProperty -Path $wuPath -Name 'SetActiveHours' -Value 1 -Type DWord -Force
echo Set-ItemProperty -Path $wuPath -Name 'ActiveHoursStart' -Value 8 -Type DWord -Force
echo Set-ItemProperty -Path $wuPath -Name 'ActiveHoursEnd' -Value 23 -Type DWord -Force
echo Set-ItemProperty -Path $wuPath -Name 'ManagePreviewBuilds' -Value 1 -Type DWord -Force
echo Set-ItemProperty -Path $wuPath -Name 'ManagePreviewBuildsPolicyValue' -Value 0 -Type DWord -Force
...
echo Set-ItemProperty -Path $auPath -Name 'RebootWarningTimeoutEnabled' -Value 1 -Type DWord -Force
echo Set-ItemProperty -Path $auPath -Name 'RebootWarningTimeout' -Value 60 -Type DWord -Force
echo Set-ItemProperty -Path $auPath -Name 'ScheduledInstallDay' -Value 0 -Type DWord -Force
echo Set-ItemProperty -Path $auPath -Name 'ScheduledInstallTime' -Value 3 -Type DWord -Force
echo.
echo # 4. Complementos do padrao PulseOS para drivers e metadados
echo $driverSearchPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DriverSearching'
echo if ^(-not ^(Test-Path $driverSearchPath^)^) { New-Item -Path $driverSearchPath -Force ^| Out-Null }
echo Set-ItemProperty -Path $driverSearchPath -Name 'SearchOrderConfig' -Value 0 -Type DWord -Force
echo Set-ItemProperty -Path $driverSearchPath -Name 'DontSearchWindowsUpdate' -Value 1 -Type DWord -Force
echo Set-ItemProperty -Path $driverSearchPath -Name 'DontPromptForWindowsUpdate' -Value 1 -Type DWord -Force
echo $deviceMetadataPath = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Device Metadata'
echo if ^(-not ^(Test-Path $deviceMetadataPath^)^) { New-Item -Path $deviceMetadataPath -Force ^| Out-Null }
echo Set-ItemProperty -Path $deviceMetadataPath -Name 'PreventDeviceMetadataFromNetwork' -Value 1 -Type DWord -Force
echo.
echo # 5. Pausar Windows Update por 7 dias
...
echo.
echo # 6. Log de execucao da tarefa
...
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"        /v "SetActiveHours"                    /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"        /v "ActiveHoursStart"                 /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"        /v "ActiveHoursEnd"                   /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"        /v "ManagePreviewBuilds"              /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate"        /v "ManagePreviewBuildsPolicyValue"   /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\DriverSearching"      /v "SearchOrderConfig"                /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\DriverSearching"      /v "DontSearchWindowsUpdate"          /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\DriverSearching"      /v "DontPromptForWindowsUpdate"       /f 2>nul
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\Device Metadata"      /v "PreventDeviceMetadataFromNetwork" /f 2>nul
